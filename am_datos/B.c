...
int calculate_point_position(double z, double alpha, double d, Polar_Coordinates *coordinates, double x0)
{
	...
	double omega;
	double r_sol = R_SOL;
	double a1, a2, a3, r2, tmp1, tmp2, tmp3;
	a1 = pow(z + R_SOL,2);
	a2 = pow(d,2);
	a3 = 2*(z+r_sol)*d*cos(alpha);
	tmp1 = 2*(z+R_SOL);
	tmp2 = tmp1*d;
	tmp3 = cos(alpha);
	r2 = sqrt(a1 - a3 + a2);
	coordinates->r =
		sqrt( pow(z + R_SOL, 2) + pow(d,2) - 2*(z + R_SOL)*d*cos(alpha));
	if(coordinates->r == 0)
	{
		coordinates->theta = .1;
		coordinates->r = 1;
		...
		omega = asin( d*sin(alpha) / coordinates->r);
		coordinates->theta = M_PI/2 - alpha - omega;
	}

//	printf( "2*(z+R_SOL)=%le d=%le cos(alpha)=%le a3=%le alpha=%le\n", tmp1, tmp2, tmp3, a3, alpha);

	printf( "z=%le d=%le alpha=%le r=%le\n theta=%le", z, d, alpha, coordinates->r, coordinates->theta);
	//----------------------------------------
/*
...
//	printf( "\nr = %f theta = %f \n", polar_field->r, polar_field->theta); 
	//Calculo br y bt
	for(i = 0; i < size; i++){
...
//	double m = 2596400000000;
	...
	B_total = sqrt( pow(my_vec.cartesian_field->bx,2) + pow(my_vec.cartesian_field->by,2) );
	fprintf(magnetic_f,"%s\n%le %le\n","z\t\tB", (*model).atm.z, my_vec.cartesian_field->bx, my_vec.cartesian_field->by, my_vec.cartesian_field->bz, B_total);
...
//	printf("Xi = %le\n", model->atm.xi);
	...
	//fprintf(magnetic_f, "%le %le %le %le %le\n", (*model).atm.z, my_vec.cartesian_field->bx, my_vec.cartesian_field->by, my_vec.cartesian_field->bz, B_total);
